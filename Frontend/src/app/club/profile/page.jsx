...
import { MapPin, Flag, FileText, Image as ImageIcon, File, User} from "lucide-react";
    ...
    <div>
      <div className="p-6 mt-0 px-4 sm:px-6 lg:px-8 mx-auto w-full max-w-6xl grid grid-cols-1 gap-8"> 
        <div className="flex items-center gap-3 mb-6">
          <h1 className="text-3xl font-bold text-[#fccc22]">ACHIEVEMENTS</h1>
          <button
        ...
        <div className="space-y-4">
          {achievements.map((ach) => (
              <div
                key={ach.id}
                className="flex items-center justify-between bg-[#34285a] relative -top-5 rounded-xl p-6 shadow-md hover:scale-[1.01] transition-transform duration-200 gap-8"
              > {/* bg-[#1C1633]/60 border border-[#3b2d5e] rounded-xl     bg-gradient-to-br from-[#0C0817] via-[#28213a] to-[#1C1633]*/}
...
<div className="p-4 flex justify-start">
  <CenteredSearch className="w-[600px]"  />
</div>



{/*Club Info*/} 
<section className="relative z-10 rounded-xl p-6 shadow-lg bg-[#1d1338]">
      <button
  ...
  {profile.profilePhoto ? (
    <Image
      ...
      width={105}
      height={105}
      className="w-32 h-32 rounded-full border-transparent shadow-[0_0_20px_2px_rgba(95,74,135,1)]"
    />
  ) : (
    <div className="w-32 h-32 flex items-center justify-center rounded-full bg-[#2b2142] shadow-[0_0_20px_2px_rgba(95,74,135,1)]">
      ...
      <div className="-mt-5 flex justify-between relative top-7 items-start">
        <div className="max-w-[650px] text-white-400 mt-8 md:mt-8">
          <p className="text-xl ml-3 mb-5">{profile.bio}</p>
        </div>
  ...
  {profile.socials?.discord && (
    <a href={profile.socials.discord} target="_blank" rel="noopener noreferrer">
      <img src="/discord (2).svg" alt="Discord" className="w-9 h-9 relative -top-2 ml-2.5 icon-glow" />
    </a>
  )}
  {profile.socials?.youtube && (
    <a href={profile.socials.youtube} target="_blank" rel="noopener noreferrer">
      <img src="/youtube (2).svg" alt="YouTube" className="w-[55px] h-[55px] relative -top-4 icon-glow" />
    </a>
  )}
  {profile.socials?.x && (
