...
export default function scrimAppointments({
  id,
  gameName,
  cover,
  status,
  ...
  return (
    ...
    >
      {/* Header image */}
      {cover ? (
        <img src={cover} alt={gameName} className="w-full h-60 object-cover" />
      ) : (
      ...
      <div className="p-4 flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="font-bold text-white text-[26px] truncate">{gameName || "Scrim"}</span>
            <span
          ...
          <div className="mt-1 text-gray-300 text-[15px] leading-snug">
            ...
            {inviteLink ? (
              <div className="truncate opacity-80">
                invite: <span className="underline">{inviteLink}</span>
              </div>
            ) : null}
